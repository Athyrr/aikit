---
name: probe
description: Bounded investigation that answers one specific technical unknown so a blocked task can resume. Returns a finding, never an implementation. Dispatch from aikit:routing-failures when the failure routes downward.
...
- **One question**, given to you in writing. If your dispatch does not contain a
  single answerable question, say so and stop — an unbounded probe is how a
  session disappears.
- **A stated bound**, given to you in writing. When it is spent, report what
  you know and what you do not. Running past it silently is the failure this
...
- **Answer** — or "no answer, the bound is spent, here is what is now excluded".
