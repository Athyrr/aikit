...
**Note:** Pass the project root as `--project-dir` so mockups persist in `vault/<project>/<feature>/brainstorm/` and survive server restarts. Without it, files go to `/tmp` and get cleaned up. Remind the user to add `.aikit/` to `.gitignore` if it's not already there.
