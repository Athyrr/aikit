...
| 2.5 Impact | the project's domain expert | its own prompt + the docs of its area | orchestrator receives `impact.md` only |
| 3-4 Plan | `aikit:planner` | spec, `impact.md`, the doc **slices** the registry routes to | the whole doc; the orchestrator does not re-read |
